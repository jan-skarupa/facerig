...
public:
    ...
    std::unique_ptr<cv::Rect> detect_face(const cv::Mat& frame) override;
...
std::unique_ptr<cv::Rect> FaceDetectorHarr::detect_face(const cv::Mat &frame)
{
    ...
    return std::make_unique<cv::Rect>(faces[0]);
}
